package com.example.dakiplataformas.service;


import com.example.dakiplataformas.builder.WalletBuilder;
import com.example.dakiplataformas.model.*;
import com.example.dakiplataformas.model.admin.TransactionResponseDto;
import com.example.dakiplataformas.repository.PaymentMethodRepository;
import com.example.dakiplataformas.repository.SettingRepository;
import com.example.dakiplataformas.util.DakiException;
...
import com.example.dakiplataformas.repository.UserRepository;
import com.example.dakiplataformas.repository.WalletTransactionRepository;
...
public class WalletService {
    ...
    public void cancelPendingRecharge(String principalName, UUID txId) {
        WalletTransaction tx = walletTransactionRepository.findById(txId)
                .orElseThrow(() -> DakiException.notFound("Transacción no encontrada"));


        if (!"pending".equalsIgnoreCase(tx.getStatus())) {
            throw DakiException.invalidState("Solo transacciones en estado pending pueden cancelarse");
        }
        ...
        if (!isOwner && !isAdmin) {
            throw DakiException.accessDenied("No autorizado para cancelar esta transacción");
        }
